...
def _transformers_importable() -> bool:
    """Probed once at collection, exactly like MASKER_AVAILABLE above: a
    python3 that can `import transformers, torch, PIL` inside a generous
    timeout. Anything else (no python3, no install, a broken env) means the
    depth path is a loud, honest skip — never a silent no-op disguised as a
    pass."""
    if shutil.which("python3") is None:
        return False
    try:
        result = subprocess.run(
            ["python3", "-c", "import transformers, torch, PIL"],
            capture_output=True,
            timeout=30,
        )
...
DEPTH_AVAILABLE = _transformers_importable()
...
@pytest.mark.skipif(
    not (MASKER_AVAILABLE and DEPTH_AVAILABLE),
    reason="needs macOS + swift (to build a cutout at all) AND python3 + transformers/torch/pillow importable (depth)",
)
def test_rig_build_and_serve_real_depth(server, upload, artifact_waiter):
    """P2 depth/parallax (docs/EMANATION-ENGINE-PLAN.md §4.3): the real path,
    a genuine cutout masked by Vision, then a genuine Depth Anything V2 Small
    depth map generated by depth.py. The model is a one-time, already-cached
    download on this machine, so this proves the real end-to-end contract,
    not a fixture — hence the generous timeout (model load + CPU/MPS
    inference on a single photo, one time)."""
    cid = create_companion(server)
    seed_processed_photo(server, upload, artifact_waiter, cid, ONI_PHOTO, physical=["orange tabby"])

    status, body = api(server, "POST", f"/api/companions/{cid}/rig", timeout=240)
    assert status == 200, body
    rig = body["rig"]
    assert rig["depth_url"] == f"/api/companions/{cid}/rig/depth"

    with urllib.request.urlopen(f"{server.base_url}/api/companions/{cid}/rig/depth") as r:
        assert r.status == 200
        assert r.headers.get("Content-Type") == "image/png"
        data = r.read()
        assert data[:8] == b"\x89PNG\r\n\x1a\n"
    ...
    stored = con.execute("SELECT rig_json FROM companions WHERE id=?", (cid,)).fetchone()[0]
    con.close()
    assert json.loads(stored)["depth_url"] == rig["depth_url"]


@pytest.mark.skipif(
    DEPTH_AVAILABLE,
    reason="transformers/torch/pillow importable on this machine; nothing to assert here",
)
def test_rig_build_without_depth_libs(server, upload, artifact_waiter):
    """When the depth toolchain is absent, the honest answer is Phase-1-only:
    masking (independent of depth) still produces a real rig, the descriptor
    simply carries no depth_url, and GET /rig/depth 404s — never a silent
    fake parallax."""
    if not MASKER_AVAILABLE:
        pytest.skip("needs macOS + swift on PATH to build a rig at all")
    cid = create_companion(server)
    seed_processed_photo(server, upload, artifact_waiter, cid, ONI_PHOTO, physical=["orange tabby"])
    ...
    assert status == 200, body
    rig = body["rig"]
    assert "depth_url" not in rig

    try:
        urllib.request.urlopen(f"{server.base_url}/api/companions/{cid}/rig/depth")
        raise AssertionError("expected 404")
    except urllib.error.HTTPError as e:
        assert e.code == 404
        assert json.loads(e.read())["error"] == "depth_missing"
